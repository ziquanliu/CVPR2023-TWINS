...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
        ...
        if make_adv:
            ...
            inp = adv
        inp_resize=F.interpolate(inp, 224)
        normalized_inp = self.normalizer(inp_resize)
        #print(normalized_inp.size())
    ...
    def forward_IN(self, inp, target=None, make_adv=False, with_latent=False,
                fake_relu=False, no_relu=False, with_image=False, **attacker_kwargs):
        """
        Main function for running inference and generating adversarial
        examples for a model.
        ...
        #print(inp[0,0,0:10,0])
        normalized_inp = self.normalizer(inp)
        #print(normalized_inp[0,0,0:10,0])
        #print(normalized_inp.size())
        ...
        output = self.model.forward_IN(normalized_inp, with_latent=with_latent,
                                fake_relu=fake_relu, no_relu=no_relu)
        if with_image:
            return (output, inp)
        return output
